...
struct GridCellRecord: Codable, Hashable {
    let x: Int
    let z: Int
}

struct WorldPointRecord: Codable, Hashable {
    let x: Double
    let z: Double
}

struct RobotPoseRecord: Codable {
    let x: Double
    let z: Double
    let fx: Double
    let fz: Double
...
struct NavigationMapData: Codable {
