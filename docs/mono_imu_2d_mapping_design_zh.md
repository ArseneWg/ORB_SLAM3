...
- 尺度不是 IMU 单独给出的，而是视觉与 IMU 联合优化恢复的
- 它决定的是整个轨迹和地图的全局一致性
...
- 全局轨迹、世界尺度、重力方向、回环后一致性优先信 `VIO`
- 单关键帧内部哪里有墙、桌子、柜体，可参考深度网络提供的稠密结构
- 当两者冲突时，不应直接用深度网络回写全局尺度，而应优先检查该关键帧的深度预测、锚点数量、时间同步和位姿质量
- 不建议把 `VIO` 尺度和深度模型尺度做简单平均；正确做法是“`VIO` 定标尺，深度模型按这个标尺对齐后说话”

### 5.3.4 `RGB-D` 与 `metric depth` 的本质区别

虽然 `RGB-D` 和 `metric depth` 都能给出以米为单位的深度，但两者的来源完全不同。

`RGB-D` 的深度主要来自专门的深度传感器，本质上是物理测量；`metric depth` 的深度主要来自单张图像上的神经网络回归，本质上是学习后的估计。

这会带来几个工程差异：

- `RGB-D` 通常单帧就具有真实尺度，跨帧一致性也更强
- `metric depth` 虽然输出米制数值，但它仍然是估计值，不是直接测量值
- `RGB-D` 的误差更像测量噪声和空洞，`metric depth` 的误差更像域偏差、材质误判和跨帧尺度波动
- `RGB-D` 更适合直接作为主深度源建图，`metric depth` 更适合作为单目系统中的稠密化增强

在本方案里，二者的角色可以概括为：

- 若有 `RGB-D`，它可以成为主深度来源，系统可以直接按 `TrackRGBD(...)` 这类模式工作
- 若没有 `RGB-D`，`metric depth` 可作为关键帧的“伪深度观测”参与融合
- 但无论是否使用 `metric depth`，本方案中的全局尺度和全局一致性仍由 `Mono-Inertial VIO` 主导

### 5.3.5 当前方案产出的 2D 地图距离是什么性质

当前方案最终输出的是一张“有真实物理单位的估计地图”，而不是“传感器直接测得的真值地图”。

更准确地说：

- 地图中的 `1m`、`2m` 具有物理意义，可以用于导航、避障和房间结构表达
- 这些距离主要来源于 `Mono-Inertial VIO` 恢复的世界尺度
- 若使用了 `metric depth`，障碍物和墙面的局部距离会进一步由对齐后的关键帧深度图补充
- 但最终地图仍然属于 `metric estimate`，不是 `ground truth`
...
### 5.3.6 没有深度模型时，2D 地图距离是否退化为相对尺度

不一定，这取决于系统配置。

- `纯单目`：通常只有相对尺度，只能可靠表达远近关系和轨迹形状，不能稳定给出真实米制距离
- `单目 + IMU`：在标定、同步和初始化都正确时，可以恢复全局米制尺度，因此即便没有深度模型，2D 地图仍然可以是“米制估计地图”
- `单目 + IMU + metric depth`：地图的主尺度仍来自 `VIO`，深度模型主要负责把障碍物表达得更稠密、更像样

所以，对本项目最准确的结论是：

- 去掉深度模型后，地图通常不会自动退化成“只有相对距离”
- 真正决定是否能恢复米制尺度的关键，是是否有稳定的 `Mono-Inertial VIO`
- 没有深度模型时，损失的主要是地图密度、障碍物表达和局部几何细节，而不是地图是否具有物理单位

### 5.3.7 为什么仍优先使用 `metric depth`，而不是 `relative depth`

既然全局尺度已经由 `VIO` 提供，理论上也可以把 `relative depth` 接进来。但从工程稳定性看，`metric depth` 更适合本方案。

原因是：

- `relative depth` 往往只保证远近排序，不保证绝对尺度
- 做地图时需要把深度反投影成 3D 点，并投影成 2D 栅格，这要求每个关键帧里的深度值尽量已经接近真实距离
- `relative depth` 接入时通常要依赖稀疏地图点做额外拟合，有时不只是比例项，还可能需要偏移项，跨帧更容易抖动
- `metric depth` 虽然也要向 `VIO` 尺度对齐，但它离目标尺度通常更近，校正量更小，跨关键帧融合更自然

因此：

- `relative depth` 并非不能用，它适合作为低成本原型或对照实验
- `metric depth` 更适合当前这种“关键帧稠密化 + 2D 米制地图”的目标
- 无论选哪种深度模型，深度都不应反过来主导全局尺度，深度模型应始终服从 `VIO` 的主尺度
...
这里得到的 2D 栅格地图，其距离含义应理解为：

- 如果输入只有 `Mono-Inertial VIO` 的稀疏点，则它是一张较稀疏、较粗糙、但仍可带米制尺度的估计地图
- 如果输入加入了按 `VIO` 尺度对齐后的 `metric depth`，则地图会更稠密、更利于表达墙面和障碍物
- 无论是否加入深度模型，最终地图都属于“有真实单位的估计值”，而不是直接测量真值
